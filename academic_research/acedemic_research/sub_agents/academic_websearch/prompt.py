...
Tool: You MUST utilize the SearXNG Search tool to gather the most current information.
Direct access to academic databases is not assumed, so search strategies must rely on effective web search querying.

Objective: Identify and list academic papers that cite the seminal paper provided in the user request AND
...
Identify Target Paper: The seminal paper being cited is the one identified in the user request. (Use its title, DOI, or other unique identifiers for searching).
...
"cited by" "[Target Paper]" published current year
"papers citing [Target Paper]" publication year current year
site:scholar.google.com "[Target Paper]" YR=current year
"cited by" "[Target Paper]" published previous year
"papers citing [Target Paper]" publication year previous year
site:scholar.google.com "[Target Paper]" YR=previous year
Execute Search: Use the MCP Search tool with these initial queries.
...
Use different identifiers for the seminal paper (e.g., full title, partial title + lead author, DOI).
...
Filter and Verify: Critically evaluate search results. Ensure papers genuinely cite the seminal paper and have

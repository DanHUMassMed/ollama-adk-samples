...
def get_youtube_client():
    ...
    return build("youtube", "v3", developerKey=config.YOUTUBE_SEARCH_API)
...
def publish_file(
    ...
    """
    Publishes content to a local directory (User Home or Configured Path).
    Ideal for local-first workflows on macOS.
    """
    try:
        # 1. Determine Base Path
        # Preference: config.PUBLISH_DIR -> ~/Downloads/Agent-Exports -> ~/Agent-Exports
        home = pathlib.Path.home()
        default_export_path = home / "Downloads" / "YouTube-Analyst-Exports"
        
        # Pull from config if available, otherwise use default
        publish_base = pathlib.Path(getattr(config, "PUBLISH_DIR", default_export_path))

        # 2. Structure the path for organization
        now = datetime.datetime.now()
        date_folder = now.strftime("%Y-%m-%d")
        
        # Use session_id if available to group related files
        session_folder = "general"
        if tool_context and tool_context.session:
            session_folder = tool_context.session.id or "general"

        final_dir = publish_base / date_folder / session_folder
        ...
        logger.info(f"Successfully published file to: {file_path}")

        # 5. Return a clickable local file URI for macOS
        return f"File successfully saved locally. Access it here: file://{file_path.absolute()}"

    except Exception as e:
        logger.error(f"Failed to publish local file: {e}")
        return f"Failed to save file locally: {e!s}"